...
def write_model_manifest(target_well, all_wells, external_factors, base_out_dir):
    ...
    manifest = {
        ...
        "overlong_gap_rule": "If actual gap length > max trained M1 step, fuse only the first max_step points and use M2-only for the tail."
    }
...
class WellPredictor:
    ...
    def get_m1_context(
        ...
        """Return concatenated past+future context for M1, or None with reason.

        For normal gaps, future context starts at start_idx + selected_step because
        the selected M1 model was trained for that missing length.

        For overlong gaps, start_idx + selected_step is still inside the real
        missing segment. To implement the requested rule, future context is taken
        after the whole real gap, i.e. start_idx + actual_length.
        """
        ...
        future_start = start_idx + actual_length if overlong else start_idx + selected_step
...
def impute_one_well(
    ...
    for seg_idx, seg in enumerate(segments, start=1):
        ...
        if overlong:
            selected_step = predictor.max_step
            fusion_len = predictor.max_step
            gate_step = predictor.max_step
        else:
        ...
        for k, row_idx in enumerate(row_indices):
            ...
            if overlong and k >= predictor.max_step:
                if np.isfinite(m2_pred):
                    final = m2_pred
                    method = "M2_only_overlong_tail"
                    reason = "actual_length_exceeds_max_step_tail_uses_M2"
                else:
                    final = np.nan
                    method = "Skipped"
                    reason = "overlong_tail_M2_unavailable"
...
def write_output_excel(
    ...
    fills = {
        ...
        "M2_only_overlong_tail": PatternFill("solid", fgColor="D9EAF7"),
